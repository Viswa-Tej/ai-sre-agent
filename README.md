...
| Layer | Tool | Why |
|---|---|---|
| Dev environment | GitHub Codespaces | Browser-based, 60hr/mo free, Docker pre-installed |
| Kubernetes | k3d (k3s in Docker) | Real upstream k8s, spins up in 30s, runs on 4GB |
| Monitoring | kube-prometheus-stack | Industry standard, ships Grafana + rules out of box |
| AI | Google Gemini 1.5 Flash | Free tier, no card, 15 req/min, 1M tokens/day |
| CI/CD | GitHub Actions | Auto-build on push, free for public repos |
| Image registry | GitHub Container Registry | Auto-auth via GITHUB_TOKEN, no separate account |
| IaC | Terraform + GCP | Demonstrates cloud provisioning skill, free e2-micro |
...
1. **GitHub account** — free, you probably already have one
2. **Google account** — for Gemini API key (AI Studio, not GCP billing)
3. **GCP account** — only needed if you want the Terraform section; requires a card for
...
```bash
# Always commit .gitignore before anything else
git add .gitignore
...
```bash
# Codespaces doesn't ship Terraform by default, so we install it manually
...
```bash
# We need an SSH key so Terraform can inject it into the VM's authorized_keys
# -t rsa = RSA algorithm
...
```bash
# This file is gitignored — it holds your real values.
...
```bash
# Add the official Prometheus community Helm chart repository
...
```bash
# Commit and push everything to trigger the first build
...
```bash
# Create the K8s Secret for the Gemini API key.
...
```bash
# Final sanity check — verify no secrets are staged
git status
...
```bash
# Destroy the GCP VM (always do this — even free resources are good to clean up)
...
[LinkedIn](https://www.linkedin.com/in/viswa-teja-065580181/) | payamviswateja11@gmail.com
